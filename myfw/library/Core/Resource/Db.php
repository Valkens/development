...
class Core_Resource_Db
{
    ...
    public function beginTransaction() {
        if($this->transactionLevel === 0){
            $this->pdo[$this->adapter]->beginTransaction();
        }
        else{
            $this->pdo[$this->adapter]->exec("SAVEPOINT LEVEL{$this->transactionLevel}");
        }
    ...
    public function commit() {
        $this->transactionLevel--;
        if($this->transactionLevel === 0){
            $this->pdo[$this->adapter]->commit();
        }
        else{
            $this->pdo[$this->adapter]->exec("RELEASE SAVEPOINT LEVEL{$this->transactionLevel}");
        }
    }
    ...
    public function rollBack() {
        $this->transactionLevel--;
        if($this->transactionLevel === 0){
            $this->pdo[$this->adapter]->rollBack();
        }
        else{
            $this->pdo[$this->adapter]->exec("ROLLBACK TO SAVEPOINT LEVEL{$this->transactionLevel}");
        }
    }
    ...
    public function query($query, $param=null){
        ...
        $stmt = $this->pdo[$this->adapter]->prepare($query);
    ...
    */
    ...
    public function quote($string, $type=null) {
        return $this->pdo[$this->adapter]->quote($string, $type);
    }
    ...
    public function lastInsertId() {
        return $this->pdo[$this->adapter]->lastInsertId();
    }
    ...
    public function relate($model, $rmodel, $opt=null){
        ...
        if($mtype==NULL)
            throw new Exception("Model $rmodel does not relate to $class_name. This is a reverse check.");
...
//														echo "<h3>$k2 = $v2</h3>";
    ...
    public function relatedInsert($model, $rmodels){
        ...
        foreach($rmodels as $rmodel){
            ...
            if($rtype==NULL)
                throw new Exception("Model $class_name does not relate to $rclass_name");
    ...
    public static function relationType($map, $model_name, $relate_model_name){
		if (!isset($map[$model_name])) {
			throw new Exception("No relationship mapping found between '{$model_name}' and '{$relate_model_name}'");
		}
        $r1 = $map[$model_name];
        #print_r($r1);
        $rtype = NULL;
