<?php
abstract class Core_Db_Manage_Updater
{
	 ...
	 * Core_Db_Manage_Updater::latestVersion. eg. if the current version is 5 and the latestVersion
	...
	public function updateToVersion($version) {
		...
		try {
			...
			throw new Exception("Error Running Database Update : $updateTo", $ex->getMessage());
		}
	}
	...
	public function downgradeToVersion($version) {
		...
		try {
			...
			throw new Exception("Error Running Database Downgrade : $downgradeTo", $ex->getMessage());
		}
	}
	...
	public function executeVersionUpdate($version) {
		...
		if (method_exists($this, $methodName) == true) {
			...
			throw new Exception("Database upgrade function $methodName could not be found");
		}
	}
	...
	public function executeVersionDowngrade($version) {
		...
		if (method_exists($this, $methodName) == true) {
			...
			throw new Exception("Database downgrade function $methodName could not be found");
		}
	}
	...
	private function getDbEngineManager($engine) {
		...
		if ($engine == 'mysql') {
			return new Core_Db_Manage_Adapter_MySql();
		} else if ($engine == 'sqlite') {
			return new Core_Db_Manage_Adapter_Sqlite();
		} else {
