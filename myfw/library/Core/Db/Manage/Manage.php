<?php
abstract class Core_Db_Manage_Manage
{
	...
	/**
	 * A mapping of Core_Db generic datatypes to RDBMS native datatypes for columns
	...
	protected $colTypeMapping = array (
        Core_Db_Manage_Manage::COL_TYPE_BOOL		=> null,
        Core_Db_Manage_Manage::COL_TYPE_SMALLINT	=> null,
        Core_Db_Manage_Manage::COL_TYPE_INT		=> null,
        Core_Db_Manage_Manage::COL_TYPE_BIGINT	=> null,
        Core_Db_Manage_Manage::COL_TYPE_DECIMAL	=> null,
        Core_Db_Manage_Manage::COL_TYPE_FLOAT		=> null,
        Core_Db_Manage_Manage::COL_TYPE_CHAR		=> null,
        Core_Db_Manage_Manage::COL_TYPE_VARCHAR	=> null,
        Core_Db_Manage_Manage::COL_TYPE_CLOB		=> null,
        Core_Db_Manage_Manage::COL_TYPE_DATE		=> null,
        Core_Db_Manage_Manage::COL_TYPE_TIME		=> null,
        Core_Db_Manage_Manage::COL_TYPE_TIMESTAMP	=> null,
	);
    ...
    public function connect(){
        ...
        try {
            ...
            throw new Exception('Failed to open the DB connection', $ex->getMessage());
        }
    }
	 ...
	 * 	$cols = array(
	 * 		'column1' => array(
	 * 			'type'    => Core_Db_Manage_Manage::COL_TYPE_*
	 ...
	 * }}
	 *
	 * The field types are defined in the Core_Db_COL_TYPE_* definitions which are defined in the Core_Db_Manage_Manage file
	 *
	...
	public function createTable($table, $cols, $options=null) {
		...
		try {
			...
			throw new Exception("Error Creating Table : $table", $ex->getMessage());
		}
	}
	...
	protected function _sqlCreateTable($table, $columnDefinitions, $options=null) {
		...
		if ($errors) {
			throw new Exception("Could not create table", implode("\n", $errors));
		}
	...
	public function dropTable($table) {
		...
		try {
			...
			throw new Exception("Error Dropping Table : $table", $ex->getMessage());
		}
	}
	...
	public function addColumn($table, $name, array $attributes) {
		...
		try {
			...
			throw new Exception("Error Adding Column ($name) to Table ($table)", $ex->getMessage());
		}
	}
	...
	public function dropColumn($table, $name) {
		...
		try {
			...
			throw new Exception("Error Dropping Column ($name) from Table ($table)", $ex->getMessage());
		}
	}
	...
	protected function buildColumnDefinition($name, $attributes) {
        ...
        if (array_key_exists($type, $this->colTypeMapping) == false) {
            throw new Exception('Unrecognised column type specified', "$type is not a recognised database type. Please check Core_Db_Manage_Manage::colTypeMapping for supported column types");
        }

        // get the column type defenition
        switch ($type) {

        	case Core_Db_Manage_Manage::COL_TYPE_CHAR:
        	case Core_Db_Manage_Manage::COL_TYPE_VARCHAR:
	            if ($size < 1 || $size > 255) {
	                throw new Exception("Invalid Column Size ($size) for $name", "char and varchar require a column size between 1 and 255");
	            } else {
					$columnDefinition = $this->colTypeMapping[$type] . "($size)";
				}
	            break;

	        case Core_Db_Manage_Manage::COL_TYPE_DECIMAL:
	            if ($size < 1 || $size > 255) {
	                throw new Exception("Invalid Column Size ($size) for $name", "decimal requires size to be between 1 and 255");
	            } elseif ($scope < 0 || $scope > $size) {
	                throw new Exception("Invalid Column Scope ($scope) for $name", "Scope must be between 0 and the Column Size ($size)");
	            } else {
	...
	protected function checkIdentifier($type, $name) {
        ...
        $len = strlen($name);
        if ($len < 1 || $len > 30) {
            throw new DbException("Invalid Identifer for $type : $name", "Identifiers must be between 1 and 30 characters long");
        }
        ...
        if (preg_match($regex, $name) == false) {
            throw new Exception("Invalid Identifer for $type : $name", "Identifer must start with a letter a-z and contain only letters, numbers and underscores");
        }

        // must not have two or more underscores in a row
        if (strpos($name, '__') !== false) {
            throw new Exception("Invalid Identifer for $type : $name", "Identifier can not contain 2 or more underscores consecutavily");
        }
	}
}