...
class Core_Helper_Validator
{
    ...
    public function validate($data=null, $rules=null){
        ...
        foreach($data as $dk=>$dv){
            if($this->requireMode == Core_Helper_Validator::REQ_MODE_NULL_EMPTY && ($dv === null || $dv === '') ||
			   $this->requireMode == Core_Helper_Validator::REQ_MODE_NULL_ONLY  && $dv === null){
                unset($data[$dk]);
			}
        }
        ...
        foreach($data as $k=>$v){
            ...
            foreach($cRule as $v2){
                if(is_array($v2)){
					...
					$vIsEmpty = ($this->requireMode == Core_Helper_Validator::REQ_MODE_NULL_EMPTY && ($v === null || $v === '') ||
								 $this->requireMode == Core_Helper_Validator::REQ_MODE_NULL_ONLY  && $v === null) ? true : false;
                    ...
                    if($err = call_user_func_array(array(&$this, 'test'.$v2[0]), $vv) ){
                        if($this->checkMode==Core_Helper_Validator::CHECK_ALL)
                            $errors[$k][$v2[0]] = $err;
                        else if($this->checkMode==Core_Helper_Validator::CHECK_SKIP && !$vIsEmpty && $v2[0]!='optional'){
                            return $err;
                        }else if($this->checkMode==Core_Helper_Validator::CHECK_ALL_ONE)
                            $errors[$k] = $err;
                    }
                    ...
                    if(sizeof($cRule)>1){
                        ...
                        if($err = call_user_func_array(array(&$this, 'test'.$cRule[0]), $vv) ){
                            if($this->checkMode==Core_Helper_Validator::CHECK_ALL || $this->checkMode==Core_Helper_Validator::CHECK_ALL_ONE)
                                $errors[$k] = $err;
                            else if($this->checkMode==Core_Helper_Validator::CHECK_SKIP){
                                return $err;
                            }
     ...
     * @param type $displayMethod Default error message display method. use: Core_Helper_Validator::REQ_MSG_UNDERSCORE_TO_SPACE, Core_Helper_Validator::REQ_MSG_CAMELCASE_TO_SPACE, Core_Helper_Validator::REQ_MSG_THIS, Core_Helper_Validator::REQ_MSG_FIELDNAME
     * @param type $suffix suffix for the error message. Default is ' field is required'
     */
    public function setRequiredFieldDefaults( $displayMethod = Core_Helper_Validator::REQ_MSG_UNDERSCORE_TO_SPACE, $suffix = ' field is required'){
    ...
    public function getRequiredFieldDefaultMsg($fieldname){
        if($this->requiredMsgDefaultMethod==Core_Helper_Validator::REQ_MSG_UNDERSCORE_TO_SPACE)
            return ucfirst(str_replace('_', ' ', $fieldname)) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==Core_Helper_Validator::REQ_MSG_THIS)
            return 'This ' . $this->requiredMsgDefaultSuffix;        
        
        if($this->requiredMsgDefaultMethod==Core_Helper_Validator::REQ_MSG_CAMELCASE_TO_SPACE)
            return ucfirst(strtolower(preg_replace('/([A-Z])/', ' $1', $fieldname))) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==Core_Helper_Validator::REQ_MSG_FIELDNAME)
            return $fieldname . $this->requiredMsgDefaultSuffix;
    }

    public function testOptional($value){}
    public function testRequired($value, $msg){
		if ($this->requireMode == Core_Helper_Validator::REQ_MODE_NULL_EMPTY && ($value === null || $value === '') ||
			$this->requireMode == Core_Helper_Validator::REQ_MODE_NULL_ONLY  && $value === null) {
    ...
    */
    public function testDbExist($value, $table, $field, $msg=null) {
        $result = Core_Db_Model::$db->fetchRow("SELECT COUNT($field) AS count FROM " . $table . ' WHERE '.$field.' = ? LIMIT 1', array($value));
