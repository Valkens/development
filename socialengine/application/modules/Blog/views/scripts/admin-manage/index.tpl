...
<script type="text/javascript">
  en4.core.runonce.add(function(){$$('th.admin_table_short input[type=checkbox]').addEvent('click', function(){ $$('td.delete_blogentries input[type=checkbox]').set('checked', $(this).get('checked', false)); })});

  var delectSelected =function(){
    var checkboxes = $$('td.delete_blogentries input[type=checkbox]');
    var selecteditems = [];
...
<script language="javascript">
    function blogentry_good(blogentry_id,checbox){

            if(checbox.checked==true) status =1;
            else status =0;
            new Request.JSON({
              'format': 'json',
              'url' : '<?php echo $this->url(array('module' => 'blog', 'controller' => 'manage', 'action' => 'featured'), 'admin_default') ?>',
              'data' : {
                'format' : 'json',
...
<table class='admin_table'>
  ...
  <tbody>
    <?php foreach ($this->paginator as $item): ?>
      <tr>
        <td class="delete_blogentries"><input type='checkbox' class='checkbox' value="<?php echo $item->blog_id ?>"/></td>
