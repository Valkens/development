...
class Blog_Api_Core extends Core_Api_Abstract
{
  ...
  public function getBlogsSelect($params = array())
  {
    ...
    {
      $select->where($rName.'.blog_id = ?', $params['blogRss']);
    }
     ...
     if( !empty($params['date']) )
    {
        ...
        $m = $date[1];
        $d = $date[2];
        $select->where("YEAR(creation_date) = ?", $y);
        $select->where("MONTH(creation_date) = ?", $m);
        $select->where("DAY(creation_date) = ?", $d);
    }
  ...
  {
    Engine_Api::_()->getDbtable('categories', 'blog')
      ->delete(array(
   ...
   public function checkUserBecome($user_id,$blog_id)
  {
      ...
      $select   = $table->select()
                        ->where("$name.user_id = ?",$user_id)->where("$name.blog_id = ?",$blog_id);
