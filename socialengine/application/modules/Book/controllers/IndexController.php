...
class Book_IndexController extends Core_Controller_Action_Standard
{
                ...
                switch ($this->_getParam('submit'))
                {
                    ...
                    case 'update':
                        foreach (array_keys($cartSession->list) as $id) {
                            ...
                            $cartSession->list[$id] = $quantity[$id];
                        }
                        break;
    ...
    public function transactionAction()
    {
        ...
        $this->view->headTitle('My transactions page');
        ...
        $orderId = $this->_request->getParam('id', null);
        ...
        if ($orderId) {
            ...
            $this->_helper->viewRenderer->setRender('transaction_detail');
        } else {
            $table = Engine_Api::_()->getDbtable('orders', 'book');
            $select = $table->select()->where('owner_id = ?', $viewer->getIdentity());
        }

        $this->view->orders = $orders = $table->fetchAll($select);
    }
}
